...
function BookedCard(booking) {
    ...
    return (
        <Link to={`/user/booked/${booking._id}`} className='booking_card'>
            ...
            <div>
                ...
                {booking?.fullprice ? <h4>Total Price ${booking?.fullprice}</h4> :
                    <div>
                        ...
                        <p>{booking?.Username} only paid half price, The rest (${booking?.halfprice}) will be automatically charged to the same payment method on {formatDate(booking.checkout)}</p>
                    </div>}
            </div>
        </Link>
    )
