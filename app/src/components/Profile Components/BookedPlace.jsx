                ...
                {!photoGrid ? <section section className='container_place container_book_place dim_overlay '>
                    <div className="Place_info_imgs">
                        {screenWidth > 550 ? <>
                            ...
                            {Booking?.halfprice && <p className='warning'>{customer?.fullname} only paid half price, The rest (${Booking?.halfprice}) will be automatically charged to the same payment method on {formatDate(Booking.checkout)} !</p>}
                        ...
                        <div className='meet_host'>
                            <div className="place_datails_host">
                                {!isLoading ? <div className='place_details'>
                                    <h2>Meet your customer <Link to={`/user/show/${Placeuserid}`}>{customer?.fullname}</Link></h2>
                                </div> : <Skeleton width={"250px"} />}

                                {!isLoading ? <Link to={`/user/show/${Placeuserid}`} className="place_host">
                                    <img src={`http://localhost:3001/${customer?.profilepic}`} alt="" />
                                    <FontAwesomeIcon className='ratingicon ratingicon_user' icon={faStar} />
                                </Link> : <Skeleton width={"50px"} height={"50px"} borderRadius={"50%"} />}
                            </div>

                            <div className='host_info'>
                                {!isLoading ? <p>{customer.bio}</p> : <Skeleton count={3} width={"600px"} />}
                                {!isLoading ? <p>Member since {formatDate(customer.createdAt)}</p> : <Skeleton width={"150px"} />}
                                {!isLoading ? <p>
                                    You can reach out to {customer.email} if you have any questions or need assistance during your stay.
                                    Additionally, you can contact {customer.phone} for immediate support.
